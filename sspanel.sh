...
server {  
    listen 80;
    listen [::]:80;
    root /var/www/sspanels/public; # 改成你自己的路径，需要以 /public 结尾
...
# 站点名称
sed -i "s/SSPanel-UIM/飞一般的感觉/" /var/www/sspanels/config/.config.php
# 站点地址
sed -i "s/https:\/\/sspanel.host/http:\/\/$ips/" /var/www/sspanels/config/.config.php
# 用于校验魔改后端请求
...
# 设置数据库连接密码
sed -i "s/password'\]  = 'sspanel'/password'\]  = '$Database_Password'/" /var/www/sspanels/config/.config.php
# 导入数据库文件
mysql -uroot -p$Database_Password sspanel < /var/www/sspanels/sql/glzjin_all.sql;
